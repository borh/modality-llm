...
def analyze_modal_relationship(original: str, alternative: str) -> str:
    ...
    modal_semantics = {
        "can": {"category": "ability", "strength": "medium", "polarity": "positive"},
        "could": {"category": "ability", "strength": "weak", "polarity": "positive"},
        "cannot": {"category": "ability", "strength": "strong", "polarity": "negative"},
        "may": {"category": "permission", "strength": "medium", "polarity": "positive"},
        "might": {
            "category": "possibility",
            "strength": "weak",
            "polarity": "positive",
        },
        "must": {"category": "necessity", "strength": "strong", "polarity": "positive"},
        "should": {"category": "advice", "strength": "medium", "polarity": "positive"},
        "will": {
            "category": "prediction",
            "strength": "strong",
            "polarity": "positive",
    ...
    for col_idx, col_name in enumerate(columns):
        ...
        if col_name in hidden_columns:
            worksheet.set_column(
                f"{col_letter}:{col_letter}", 20, cell_format_nowrap, {"hidden": 1}
            )
                ...
                csv_rows.append(
                    {
                        ...
                        "Grammaticality_Expected": predict_grammaticality(
                            transformation_strategy
                        ),  # Use prediction instead of hardcoded "yes"
                        "Completed": 0,
                        "Annotation_Notes": f"Modal substitution: '{original_mv}' → '{alt_mv}'. Strategy: {transformation_strategy}. Please verify grammaticality and classify modal categories.",
                    }
                )
    ...
    for row in rows:
        try:
            ...
            modal_example = {
                ...
                "grammatical": grammaticality.lower(),  # Store as string instead of boolean
                "grammatical_binary": grammaticality.lower()
                in ["yes", "partial"],  # Add binary version for backward compatibility
                "annotations": {
